class Debt < ActiveRecord::Base
	...
	def self.get_not_updated_since(user,date)
		self.viewable_debts(user).where("debts.updated_at < :date", date: date)
			.where("debt_status_id != :status", status: Debts::Status.lookup(:closed)).order('debts.updated_at asc')
	end
	...
	def current_active_placement
		placements.last if is_active_placement?
	end

	def is_active_placement?
		placements.length > 0 && placements.last.active
	end

	# note that using "last" will fail the autosave of the nested attribute
	def last_placement
		self.placements[self.placements.length-1]
	end

	def current_assigned_agency
		agency = nil
		placement = self.placements.last
	...
	def status=(symbol)
		self.debt_status_id = Debts::Status.lookup(symbol)
	end
...
private
	def is_status?(symbol)
		symbol == Debts::Status.symbol(self.debt_status_id)
	end
	...
	def pre_save_updates
		...
		new_debt_status = Debts::Status.symbol(self.debt_status_id)
	...
	def populate_defaults
		self.debt_status_id = Debts::Status.lookup(:draft) if !self.debt_status_id
	end
end
