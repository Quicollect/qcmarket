class DebtsController < ApplicationController
  ...
  load_and_authorize_resource class: Debt
  ...
  def index
    ...
    @debt_statistics = {}
    pending_debts = viewable_objects.where(deleted: false).where.not(debt_status_id: Debts::Status.lookup(:closed)).where.not(debt_status_id: Debts::Status.lookup(:resolved))
    ...
    paid_debts = viewable_objects.where("debt_status_id in (#{Debts::Status.lookup(:closed)}, #{Debts::Status.lookup(:resolved)})")
  ...
  def status_change
    ...
    if (@debt.allowed_actions.include? Debts::Status.symbol(new_status_id))
        @debt.debt_status_id = new_status_id

        # note that using "last" will fail the autosave of the nested attribute
        @debt.last_placement.active = false if [:rejected, :resolved].include?(Debts::Status.symbol(new_status_id))
...
private
  ...
  def viewable_objects
    Debt.viewable_debts(current_user)
  end
  ...
  def update_details(debt)
    ...
    members.each do | member |
      ...
      cls = str.include?('debt_') ? "Debts::#{str.sub('debt_', '').classify}".constantize : str.classify.constantize
