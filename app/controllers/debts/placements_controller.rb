module Debts
	class PlacementsController < ApplicationController
		load_and_authorize_resource class: Debts::Placement
	  	before_action :set_debt

		def new
			@debt_placement = @debt.placements.new(debts_placement_params)
		...
		def create
			...
			@debt.debt_status_id = Debts::Status.lookup(:assigned)
		    @debt.placements.each do | p |
		      p.active = false
		    end

			# TODO add validations that account is agency, price model belongs to it etc.
			# that price model is enabled etc.
			@agency = Agency.find(params[:debts_placement][:agency_id])
			@price_model = @agency.price_models.find(params[:debts_placement][:price_model_id])
			...
			@agency_contract = @agency.agency_contracts.find(params[:debts_placement][:agency_contract_id])
		    ...
		    @debt.placements.new(agency_id: @agency.id,
	...
	private
	    def set_debt
	      @debt = Debt.find(params[:debt_id])
	   	...
	   	def debts_placement_params
	    	params.require(:debts_placement).permit(:agency_id, :agency_contract_id, :price_model_id)
	  	end
	end
end
